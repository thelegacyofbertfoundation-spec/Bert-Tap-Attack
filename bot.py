...
def get_rank():
    ...
    res = c.fetchall()
    conn.close()
    if not res: return "No scores yet!"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("V5.1 ONLINE. Use button to play.", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
...
if __name__ == '__main__':
    ...
    app.add_handler(CommandHandler("leaderboard", lambda u, c: u.message.reply_text(get_rank())))
